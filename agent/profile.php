<?php
    ...
    $resimg = mysqli_query($conn, "SELECT AjentProfile FROM ajent_master WHERE AJM_Id = ' " . $_SESSION['ajm_id'] . " '");
    if (mysqli_num_rows($resimg) > 0) {
        $resimg = mysqli_fetch_assoc($resimg);

        if(!empty($resimg['AjentProfile'])) {
            $image_path = $resimg['AjentProfile'];
        }
    }

    if(isset($_POST['update'])){
        $path = "profile/" . time() . "." . pathinfo($_FILES['profile_image']['name'], PATHINFO_EXTENSION);
        if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $path)) {
            if (mysqli_query($conn, "UPDATE ajent_master SET AjentProfile = '$path' WHERE AJM_Id = ' " . $_SESSION['ajm_id'] . " ' ")) {
                    ...
                    <div class="card card_border border-primary-top">
                        ...
                        <div class="card-body">
                            <form method="post" class="g-3 needs-validation row px-3" novalidate enctype="multipart/form-data">
                                <div class="col-3" >
                                    <img id='previewImg' class='img rounded' src='<?php echo $profileImage; ?>' alt='Customer Image' height='200' width='200'/>    
                                </div>    
