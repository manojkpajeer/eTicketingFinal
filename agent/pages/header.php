<?php
  ...
  $resprofile = mysqli_query($conn, "SELECT AjentProfile, FullName, AjentEmail FROM ajent_master WHERE AJM_Id = '$_SESSION[ajm_id]'");
  if (mysqli_num_rows($resprofile) > 0) {
    $resprofile = mysqli_fetch_assoc($resprofile);
    $fullName = $resprofile['FullName'];
    if (!empty($resprofile['AjentProfile'])){
      $profileImage = $resprofile['AjentProfile'];
    }
    $emailId = $resprofile['AjentEmail'];
  }
?>
