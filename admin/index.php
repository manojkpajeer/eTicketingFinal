<?php 
    ...
    if (isset($_POST['login'])) {
        ...
        $res = mysqli_query($conn, "SELECT login_master.UserPassword, staff_master.FullName, staff_master.ST_Id FROM login_master JOIN staff_master ON staff_master.EmailId = login_master.UserEmail WHERE login_master.UserEmail = '$email' AND staff_master.StaffStatus = 1 AND login_master.UserRole = 'Staff'");
        if (mysqli_num_rows($res)>0) {
            ...
            if ($password == $row['UserPassword']) {
                ...
                if (mysqli_query($conn, "INSERT INTO log_master (UserId, UserRole, IPAddress, CreateDate) 
                    VALUES ('$row[ST_Id]', 'Staff', '$ip_address', NOW())")) {
                ...
                if (isset($_SESSION['wrongPassword']) && isset($_SESSION['wrongUser'])) {
                    ...
                    if($_SESSION['wrongPassword'] && $_SESSION['wrongUser'] == $email) {
                        ...
                        if(mysqli_query($conn, "UPDATE staff_master SET StaffStatus = false WHERE ST_Id = '$row[ST_Id]'")){
