<?php 
    ...
    $eProducts = mysqli_query($conn, "SELECT * FROM event_master WHERE EM_Id = '$_GET[pid]' "); 
	...
	if (isset($_POST['checkOut'])) {
		...
		$resDateCheck = mysqli_query($conn, "SELECT * FROM event_master WHERE EventStatus = 'Publish' AND EM_Id = '$_GET[pid]' AND BookingStatus = true");
					...
					if ((($quantity_avail - $quantity_sold) - $cart_quantitya) >= 0) {
						...
						if (empty($basketID)) {
							...
							if (mysqli_query($conn, "INSERT INTO sales_data (CategoryId, TypeId, Quantity, EventId, Status, BusketId, DateCreate) VALUES ('$cart_categorya', '$cart_typea', '$cart_quantitya', '$cartEventIda', 0, '$basketID', NOW()) ")) {
								...
								echo "<script>location.href='make-payment.php?bsk=$basketID&pid=$_GET[pid]';</script>";
							} else {
								echo "<script>alert('Oops, Unable to process..');</script>";
							}
				...
				<div class="w3l-features-photo-7_top-right">
					<div class="galleryContainer">
						...
						<div class="thumbnails">
							<label class="thumbnailImage " for="c1"><img src="<?php if (empty($eProducts['Image2'])) { echo './superadmin/event-image/product-preview.png'; } else { echo './superadmin/' . $eProducts['Image2']; }?>"
                                            ...
                                            <li class="cd-cart__product">
                                                <div class="cd-cart__details">
                                                    ...
                                                    <div class="cd-cart__actions">
                                                        <a href="product-single.php?pid=<?php echo $_GET['pid'];?>&delete_cart_item&cart_categorys=<?php echo $categoryID;?>&cart_types=<?php echo $typeID;?>&cartEventIds=<?php echo $eventID;?>" class="cd-cart__delete-item text-danger">Delete</a>
