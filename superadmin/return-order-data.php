<?php
    ...
    if (isset($_GET['return'])) {
        ...
        if ($api->returnTicket($_GET['oid'], $_GET['tlt'])) {

            if(mysqli_query($conn, "UPDATE sales_master SET SalesStatus = 'Returned' WHERE OrderId = '$_GET[oid]'")) {

                echo "<script>alert('Yay, Ticket returned successfully..');location.href='return-order-data.php?id=$eid';</script>";
            } else {

                echo "<script>alert('Oops, Ticket returned but unable to update order status..');location.href='return-order-data.php?id=$eid';</script>";
            }
        } else {
            echo "<script>alert('Oops, Unable to return ticket..');location.href='return-order-data.php?id=$eid';</script>";
        }
    }
?>
            ...
            <div class="row">
                <div class="col-lg-12 mb-4">
                    <div class="card card_border p-4 border-primary-top">
                        <h3 class="card__title position-absolute">Return Order - Order List</h3>
                            ...
                            <table id="meastroTable" class="display" style="width:100%">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $resd6 = mysqli_query($conn, "SELECT BasketId, OrderId, DateCreate, SM_Id, SalesStatus FROM sales_master WHERE EventId = '$_GET[id]' ORDER BY SM_Id DESC");
                                        if (mysqli_num_rows($resd6) > 0) {
                                            ...
                                            while($rowd6 = mysqli_fetch_assoc($resd6)) {
                                                ...
                                                ?>
                                                <a href="return-order-data.php?return=true&id=<?php echo $_GET['id'];?>&oid=<?php echo $orderId;?>&tlt=<?php echo $grandTotal;?>" onClick="return confirm('Are you sure you want to return?')"><i class="fa fa-undo"></i></a>
                                                <?php
