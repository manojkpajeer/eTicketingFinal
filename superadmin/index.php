<?php 
    ...
    if (isset($_POST['login'])) {
        ...
        $res = mysqli_query($conn, "SELECT login_master.UserPassword, admin_master.FullName, admin_master.AM_Id FROM login_master JOIN admin_master ON admin_master.AdminEmail = login_master.UserEmail WHERE login_master.UserEmail = '$email' AND admin_master.AdminStatus = 1 AND  login_master.UserRole = 'Admin'");
        if (mysqli_num_rows($res)>0) {
            ...
            if ($password == $row['UserPassword']) {
                ...
                if (mysqli_query($conn, "INSERT INTO log_master (UserId, UserRole, IPAddress, CreateDate) 
                    VALUES ('$row[AM_Id]', 'Admin', '$ip_address', NOW())")) {
                ...
                if (isset($_SESSION['wrongPassword']) && isset($_SESSION['wrongUser'])) {
                    ...
                    if($_SESSION['wrongPassword'] && $_SESSION['wrongUser'] == $email) {
                        ...
                        if(mysqli_query($conn, "UPDATE admin_master SET AdminStatus = false WHERE AM_Id = '$row[AM_Id]'")){
